  ...
  <script>
    ...
    //   const bsCreate = new bootstrap.Modal(modalCreate);
      ...
      let dataTable = $('#Datatable').DataTable({
        ...
        pageLength: 10,
        ajax: "{{ route('backend.proseskontrak.index') }}",
        columns: [
          ...
          { data: 'status',
            ...
            orderable: false,
              render: function(columnData, type, rowData, meta) {
                return ' <span class="badge badge-dark">'+columnData+'</span>';
              }
          },
       ...
       $("#tgl_buat").flatpickr({
        ...
        $('#formUpdate').attr('action', '{{ route("backend.proseskontrak.index") }}/' + data.id);
      });
    ...
    //   });
      modalEdit.addEventListener('show.bs.modal', function (event) {
        ...
        this.querySelector('#formUpdate').setAttribute('action', '{{ route("backend.proseskontrak.index") }}/' + event.relatedTarget.getAttribute('data-bs-id'));
      });
      modalEdit.addEventListener('hidden.bs.modal', function (event) {
        ...
        $(this).find('#tgl_buat').val('');
        $('#select2Paket').prop( "disabled", false );
        this.querySelector('#formUpdate').setAttribute('href', '');
      });
      modalDelete.addEventListener('show.bs.modal', function (event) {
        let button = event.relatedTarget;
        let id = button.getAttribute('data-bs-id');
        this.querySelector('.urlDelete').setAttribute('href', '{{ route("backend.proseskontrak.index") }}/' + id);
      });
